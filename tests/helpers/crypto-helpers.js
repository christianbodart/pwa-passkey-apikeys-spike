 ...
 * Works directly with Web Crypto API as used in the tests
 */
 ...
 * @returns {Promise<{original: string, decrypted: string, matches: boolean, encrypted: ArrayBuffer, iv: Uint8Array}>}
 */
...
/**
 * Get common test data patterns
 * @returns {Object} Test data sets
 */
export function getTestData() {
  return {
    simple: 'sk-test1234567890abcdef',
    openai: 'sk-proj-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnop123456',
    anthropic: 'sk-ant-api03-xyz123-abcdef',
    special: 'test-key-with-special!@#$%^&*()_+-=[]{}|;:,.<>?',
    empty: '',
    unicode: '🔐 secret-key-測試-🔑',
    long: 'a'.repeat(1000)
  };
}

/**
 * Export key to JWK format (for compatibility)
 * @param {CryptoKey} key - Key to export
 * @returns {Promise<JsonWebKey>}
 */
export async function exportTestKey(key) {
  return await crypto.subtle.exportKey('jwk', key);
}

/**
 * Import key from JWK format
 * @param {JsonWebKey} jwk - JWK to import
 * @param {boolean} extractable - Whether key should be extractable
 * @returns {Promise<CryptoKey>}
 */
export async function importTestKey(jwk, extractable = true) {
  return await crypto.subtle.importKey(
    'jwk',
    jwk,
    { name: 'AES-GCM', length: 256 },
...
/**
 * Test key export/import portability
 * @param {CryptoKey} originalKey - Key to test
 * @returns {Promise<CryptoKey>}
 */
export async function testKeyPortability(originalKey) {
  const jwk = await exportTestKey(originalKey);
  return importTestKey(jwk);
}
